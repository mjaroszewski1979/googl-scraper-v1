...
class GoogleLinkScraper(GoogleScraper):
    ...
    def get_links(self, website, word, n_pages):
        for page in range(1, n_pages):
            
            # Creating a response object
            response = self.session.get("https://www.google.com//search?q=site:https://" + website + ' ' + word + "&start=" + str((page - 1) * 10))
    ...
    def get_csv_links(self, website, word, n_pages):

        my_list = []

        for item in self.get_links(website, word, n_pages):
            if website in item:
                my_list.append(item)
...
class GoogleStatsScraper(GoogleScraper):
    ...
    def get_stats(self, website, word):
    
        # Creating a response object
        response = self.session.get("https://www.google.com//search?q=site:https://" + website + ' ' + word )
    ...
    def get_csv_stats(self, website, word):
    
        my_list = []

        for item in self.get_stats(website, word):
            my_list.append(website + ' ' + word + ':' + ' ' + item)
